...
data <- readr::read_csv(file = here("LM027", "LM027-7_OD450.csv"), locale=locale(encoding="latin1"))

samplenames <- c("A549mito_max", "HsaLung_DF4", "A549mito_mid", "HsaLung_DF8", "A549mito_min", "MmuLungWT_DF1", "RnoMito_max", "MmuLungWT_DF2", "RnoMito_mid",   "MmuLungWT_DF4", "RnoMito_min", "MmuLungWT_DF8", "HsaLung_DF1", "MmuLungALDH2KO_DF1", "HsaLung_DF2",  "blank_NA")
...
test1 <- (data_norm[grep(pattern = "HsaLung", data_norm$sample),] %>% dplyr::filter(DF != "DF8")) %>%
  ggplot() +
#  ylim(0,0.23) +
  ...
  ggtitle("LM027-7 ALDH2 activity assay: human lung lysate") +
...
#  labs(x = "x axis title", y = "y axis title") +
  ...
  theme(axis.text.x=element_blank(), axis.ticks.x=element_blank(), axis.title = element_text(size = 25), axis.text.y = element_text(size = 20)) +
  ggtitle("LM027-7 ALDH2 activity assay: mouse lung lysate") +
...
ggsave(plot = mod_plots, filename = here("LM027", "LM027-7_modified_plot.png"), device = "png", height = 8, width = 17)
ggsave()
ggsave()
...
pRatmito <- (data_norm[grep(pattern = "RnoMito", data_norm$sample),]) %>%
 ...
 # ylim(0,0.23) +
  ...
  ggtitle("LM027-7 ALDH2 activity assay: rat mitocondrial extract") +
...
ggsave(plot = cowplot, filename = here("LM027", "LM027-7_plot.png"), device = "png", height = 8, width = 20)
