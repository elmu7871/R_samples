...
data <- readr::read_csv(file = here("LM051", "LM051-4_OD450.csv"), locale=locale(encoding="latin1"))

samplenames <- c("nodetergent_rep1",
                 "nodetergent_rep2",
                 "nodetergent_rep3",
                 "NP40_rep1", 
                 "NP40_rep2", 
...
data_norm <- pivot_longer(data_norm, 
                          cols = nodetergent_rep1 :Rnomito_NA, 
                          names_to = "sample")
...
data_norm$sample <- factor(data_norm$sample, levels = c("nodetergent", "NP40", "Rnomito"))
...
```{r QC plot raw data}
data_raw <- readr::read_csv(file = here("LM051", "LM051-4_OD450.csv"), locale=locale(encoding="latin1"))
...
data_raw <- pivot_longer(data_raw, 
                          cols = nodetergent_rep1:blank_NA, 
                          names_to = "sample")
...
data_raw$sample <- factor(data_raw$sample, levels = c("nodetergent", "NP40", "Rnomito", "blank"))

pQC_raw <- ggplot(data_raw) +
  geom_line(aes(x = Time, y = value, color = sample, group = interaction(sample, rep)), size = 1) +
  theme_prism() +
  theme(axis.text.x=element_blank(), axis.ticks.x=element_blank()) +
  ggtitle("LM051-4 raw data QC")

ggsave(plot = pQC_raw, filename = here("LM051", "LM051-4_rawdata.png"), device = "png")
```
...
data_activity$sample <- factor(data_activity$sample, levels = c("nodetergent", "NP40", "Rnomito"))
```
...
activityplot  <- data_activity %>%
  ...
  ggtitle("LM051-4 ALDH2 activity detergent test") +
  labs(caption = "addition of 1% NP-40 improves data spread") +
  scale_x_discrete(labels= c("no detergent", "NP-40 added"))
  

ggsave(plot = activityplot, filename = here("LM051", "LM051-4_activityPlot.png"), device = "png", width = 6, height = 5)
