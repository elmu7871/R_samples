...
dapi <- read_csv(file = here("LM044", "LM044-11rep4_DAPI.csv"), locale=locale(encoding="latin1"))[,c(2,6)]
colnames(dapi) <- c("sample", "dapi")

sabgal <- read_csv(file = here("LM044", "LM044-11rep4_SABGalabs.csv"), locale=locale(encoding="latin1"))[,c(2,4)]
...
plot <- ggplot(plotdata_summ) +
  ...
  ggtitle("LM044-11rep4 quantification") +
  theme_prism() +
  facet_grid(~bleo)+
...
ggsave(plot = plot, filename = "LM044-11rep4_quant.png", path = here("LM044"), device = "png", width = 5, height = 4)
ggsave(plot = pNorm, filename = "LM044-11rep4_quant_bleonorm.png", path = here("LM044"), device = "png", width = 5, height = 4)
write_csv(plotdata_summ, file = here("LM044", "LM044-11_data", "LM044-11rep4_processeddata.csv"))
